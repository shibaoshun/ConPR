...
if ~t % forward map:
   if size(M,3)>1
     ...
     B =myfft( conj(M) .* reshape(repmat(Z,[1 nM]), n1, n2, nM) );  % Input is n1 x n2 image, output is n1 x n2 x nM array
...
%      Y=Z.*S(:);      
      Y=reshape(Y,size(S));
      B =  mean( M .* myifft(Y), 3);   %numel(Z) * mean( M .* myifft(Z.*S), 3);  % Input is n1 x n2 X nM array, output is n1 x n2 image
      %B =  mean( M .* myifft(Z.*S), 3);  
   end
end